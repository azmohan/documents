...
1. 加密客户端未正常工作。请执行`usec`查看，内核升级、未登陆账号密码都可能导致该问题。
2. 加密客户端正常。但是源码所在路径没有被设置为加密目录。默认情况下加密客户端只加密`/root`目录与`/home`目录，如果你的代码被放置在其他目录下，如`/mnt`下，可能出现可以拉取代码，但是打开代码乱码的情况。
